...
export function createReasoningNode(
  ...
  return async (state: AgentStateType): Promise<Partial<AgentUpdateType>> => {
    const systemParts: string[] = [
      ...
      "",
      "After reaching your verdict, use `propose_config_change` to recommend a concrete action.",
      "Call it at most once per analysis. The change will NOT execute immediately — it requires human approval.",
      "",
      "- **Kill it** → propose `set_status` with status `stopped`",
      "- **Ship it** → propose `update_feature_flag` (enable the winning variant) or `update_weights` (shift traffic)",
      "- **Keep running** → typically no config change needed",
      "",
      "Include the proposal in your final conclusion message, then call the tool.",
      "",
